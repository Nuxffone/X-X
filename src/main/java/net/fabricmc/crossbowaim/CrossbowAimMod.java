...
public class CrossbowAimMod implements ClientModInitializer{public static final String MOD_ID="crossbowaim";public static final Logger LOGGER=LoggerFactory.getLogger(MOD_ID);public static KeyMapping keyToggleTrajectory;public static KeyMapping keyToggleAutofire;@Override public void onInitializeClient(){ModConfig.load();LOGGER.info("CrossbowAim loaded");keyToggleTrajectory=KeyBindingHelper.registerKeyBinding(new KeyMapping("key.crossbowaim.toggle_trajectory",GLFW.GLFW_KEY_V,"category.crossbowaim"));keyToggleAutofire=KeyBindingHelper.registerKeyBinding(new KeyMapping("key.crossbowaim.toggle_autofire",GLFW.GLFW_KEY_B,"category.crossbowaim"));ClientTickEvents.END_CLIENT_TICK.register(CrossbowAimMod::onClientTick);}private static void onClientTick(Minecraft client){if(client.player==null)return;while(keyToggleTrajectory.consumeClick()){ModConfig.trajectoryEnabled=!ModConfig.trajectoryEnabled;ModConfig.save();client.player.displayClientMessage(Component.translatable("crossbowaim.config.toggle_trajectory",ModConfig.trajectoryEnabled?"ON":"OFF"),true);}while(keyToggleAutofire.consumeClick()){ModConfig.autofireEnabled=!ModConfig.autofireEnabled;ModConfig.save();client.player.displayClientMessage(Component.translatable("crossbowaim.config.toggle_autofire",ModConfig.autofireEnabled?"ON":"OFF"),true);}TrajectoryRenderer.renderTrajectory();AutoFireHandler.onClientTick(client);}}