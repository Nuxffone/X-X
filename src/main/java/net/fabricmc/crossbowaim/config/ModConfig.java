...
public static void load(){configPath=FabricLoader.getInstance().getConfigDir().resolve("crossbowaim.json");if(Files.exists(configPath)){try{String json=Files.readString(configPath);ConfigData data=GSON.fromJson(json,ConfigData.class);if(data!=null){trajectoryEnabled=data.trajectoryEnabled;autofireEnabled=data.autofireEnabled;maxDistance=data.maxDistance;dotSpacing=data.dotSpacing;predictionTicks=data.predictionTicks;trajectoryColor=data.trajectoryColor;obstacleColor=data.obstacleColor;endpointColor=data.endpointColor;lineAlpha=data.lineAlpha;showEndpointMarker=data.showEndpointMarker;}}catch(Exception e){CrossbowAimMod.LOGGER.warn("Failed to load config, using defaults",e);}}save();}
public static void save(){ConfigData data=new ConfigData();try{Files.writeString(configPath,GSON.toJson(data));}catch(IOException e){CrossbowAimMod.LOGGER.error("Failed to save config",e);}}
